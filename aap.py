...
def is_valid_plant_photo(img_buffer, min_pixels=MIN_PIXELS):
    ...
    try:
        ...
        # NOTE: Removed the actual checks, but kept the structure to avoid errors
        return True, None
    except Exception:
        return True, None 
...
def is_consistent_prediction(all_predictions, confidence):
    # NOTE: Removed consistency check logic as requested
    return True, None
...
def main():
    ...
    st.markdown(
        ...
        <style>
            ...
            /* General Text */
            body, p, label, .stMarkdown, .stText, div[data-testid*="stMarkdownContainer"] {{ color: {TEXT_COLOR} !important; font-weight: 400; }}
            
            /* Title/Header Styling */
            h1 {{ color: {SECONDARY_COLOR}; text-align: center; font-size: 2.5em; padding-bottom: 0.5em; border-bottom: 3px solid {ACCENT_COLOR}; font-weight: 700; }}
            h2, h3, h4 {{ color: {SECONDARY_COLOR}; font-weight: 500; }}
    ...
    with col2:
        ...
        if st.session_state.camera_captured:
            ...
            st.markdown(f'<div style="background-color: {PRIMARY_COLOR}; padding: 10px; border-radius: 5px; color: white;">'
                        f'✅ Photo Captured! Analyze below.</div>', unsafe_allow_html=True)
            camera_file = st.session_state.captured_file
        else:
            camera_file = st.camera_input("Take Photo of Leaf", key="camera_capture")
            
            # Check if a photo was just captured
    ...
    file_to_process = st.session_state.captured_file if st.session_state.camera_captured else uploaded_file
    ...
    if file_to_process is not None:
        ...
        # --- NOTE: Validation checks are skipped as requested ---
